...
# ---------------- DEFAULTS ----------------
DEFAULT_OUT_DIR = Path(r"D:\atres\PREFLIGHT\output")
...
def norm_blur(tile_gray: np.ndarray, tile_mag: np.ndarray, tile_edges: np.ndarray, P: Dict[str, Any]) -> float:
    ...
    denom = (0.6 * er + 0.4 * (mg / 20.0)) + float(P["NORM_EPS"])
...
def run_qc(img_path, P, status_cb=None):
    ...
    img_full = cv2.imread(img_path, cv2.IMREAD_COLOR)
    if img_full is None:
        raise ValueError(f"No se puede cargar la imagen: {img_path}")
    ...
    DEDUPE_DIST = int(TILE * float(P["DEDUPE_DIST_FACTOR"]))
    ...
    for i, (x, y, size, risk, kind) in enumerate(picked, start=1):
        ...
        fx = max(0, min(fx, img_full.shape[1]-1))
        fy = max(0, min(fy, img_full.shape[0]-1))
        fs = max(1, min(fs, min(img_full.shape[1]-fx, img_full.shape[0]-fy)))
        crop = img_full[fy:fy+fs, fx:fx+fs]
